...
def show_model_manager():
    ...
    while running:
        ...
        # Delete
        del_color = (150, 50, 50) if selected_index != -1 else (80, 40, 40)
        pygame.draw.rect(screen, del_color, btn_delete)
        pygame.draw.rect(screen, config.WHITE, btn_delete, 2)
        del_text = font.render("Delete Selected", True, config.WHITE)
        screen.blit(del_text, (btn_delete.centerx - del_text.get_width()//2, btn_delete.centery - del_text.get_height()//2))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not clicked_item and my < config.SCREEN_HEIGHT - 100 and my > 100:
                     selected_index = -1
                ...
                if btn_organize.collidepoint((mx, my)):
                    # Run organization
                    screen.fill(config.BLACK)
                    msg = font.render("Organizing Models...", True, config.WHITE)
                    screen.blit(msg, (config.SCREEN_WIDTH//2 - msg.get_width()//2, config.SCREEN_HEIGHT//2))
                    pygame.display.flip()
                    
                    model_manager.organize_models(dry_run=False)
                ...
                if btn_delete.collidepoint((mx, my)):
                    if selected_index != -1:
                        model_to_delete = models[selected_index]
                        try:
                            os.remove(model_to_delete)
                            del models[selected_index]
                            selected_index = -1
